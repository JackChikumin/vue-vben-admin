...
export const LockBackgroundOptions = [
  { value: LockOptionsEnum.Specify, label: t('layout.setting.lockTypeSpecify') },
  { value: LockOptionsEnum.RANDOM, label: t('layout.setting.lockTypeRandom') },
];

const getImage = import.meta.globEager(`../../../assets/images/lock/**.jpg`);
...
Object.keys(getImage).forEach((key) => {
  const name = getImage[key].default.replace(/[^0-9]/gi, '');
  LockTypeList.push({
    title: t('layout.setting.lockTypeTitle'),
    path: getImage[key].default,
    type: name,
  });
});
