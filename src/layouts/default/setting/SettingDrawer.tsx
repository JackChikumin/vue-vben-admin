...
export default defineComponent({
  ...
  setup(_, { attrs }) {
    ...
    const { getLockImage, getLockType, getHorizontal } = useLockBackgroundSetting();
    ...
    function renderLock() {
      return (
        <>
           <LockPicker
              lockTypeList={LockTypeList}
              handler={(item: typeof LockTypeList[0]) => {
                baseHandler(HandlerEnum.LOCKIMAGE, {
                  path: item.path,
                  type: item.type,
                  split: unref(getHorizontal.value) ? false : undefined,
                });
              }}
              def={unref(getLockImage.value)}
            />
            <SelectItem
              title={t('layout.setting.lockTypeName')}
              event={HandlerEnum.LOCKTYPE}
              def={unref(getLockType.value)}
              options={LockBackgroundOptions}
              disabled={!unref(LockOptionsEnum.Specify)}
            />
        </>
      );
    }
    ...
    return () => (
      ...
      >
        ...
        {renderSiderTheme()}
        <Divider>{() => t('layout.setting.lockTypeTitle')}</Divider>
        {renderLock()}
